...
<style>
.StkMgtListOverlay{width:100%;height:100%; background-color:rgb(115,115,115,0.2);
position:fixed;top:0px;  }
.StkMgtListWrap{border-radius: 19px; padding:10px; border: solid 5px #543ad4ab; background-color:#fdfdfd; position:fixed; left:150px; top:-100px; margin:0 auto;text-align: center; margin-top:200px; width: 700px; z-index:20;}
.wrap{width:100%;}
.sort{text-align: left;}
header{margin-top:30px;}
.title{text-align: center; font-size: 20px; font-weight: bold;}
nav{width:100%; height: 50px;}
.Stksearchbar{ font-size:11px; height: 20px; border-radius: 15px; border: none; background-color: #f1f1f187; box-shadow: 1px 1px 1px 1px #bfbfbf6e; padding-left:8px;}
.Stksearchbar:focus{outline: none;}
.StksearchBtn{ font-size: 11px; border: none; border-radius: 7px; height: 22px;  background-color: #f9dcdcd4;}
.StksearchSet{ font-size:11px; height: 22px; border-radius: 5px; position: relative; top: 1px; left: 5px;}
...
.StkMgtTable{ cursor:default; width:100%; height:400px; overflow-x:scroll; resize:vertical; border: solid 0.5px gray;}
...
<c:choose>
	...
	<c:otherwise>
	<div class="StkMgtListOverlay"></div>
	<div class="StkMgtListWrap">
	<script type="text/javascript">
...
//검색 기능
		...
		$(document).on('keyup','.Stksearchbar', function(event) {
			if(event.keyCode == 13){
				word = $('.Stksearchbar').val();
				option = $('.StksearchSet option:selected').val();
				searchStk(word,option,"");
			}
		});
		
		$(document).on('click','.StksearchBtn', function() {
			word = $('.Stksearchbar').val();
			option = $('.StksearchSet option:selected').val();
			searchStk(word,option,"");
		});
...
//상품 선택
	$(document).on('dblclick','.StkMgtTable_View td', function() {
		code = select.children('input[name="stk_code"]').val();
		$.ajax({
			url:"SlipMgtStkSelectOne.do",
			type:"post",
			data:{StkCode:code},
			error: function() {
				alert("연결 오류");
		...
		<div class="wrap">
			...
			<nav>
				<div class="sort">
					<input class="Stksearchbar" type="text" name="searchbar" value="" placeholder="Search">
					<input class="StksearchBtn" type="button" name="searchBtn" value="검색" onclick="">
					<select class="StksearchSet">
