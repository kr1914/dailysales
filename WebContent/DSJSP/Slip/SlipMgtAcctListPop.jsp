...
.AcctMgtListOverlay{width:100%;height:100%; background-color:rgb(115,115,115,0.2);
position:fixed;top:0px;  }
.AcctMgtListWrap{border-radius: 19px; padding:10px; border: solid 5px #543ad4ab; background-color:#fdfdfd; position:fixed; left:150px; top:-100px; margin:0 auto;text-align: center; margin-top:200px; width: 700px; z-index:20;}
.wrap{width:100%;}
.sort{text-align: left;}
header{margin-top:30px;}
.title{text-align: center; font-size: 20px; font-weight: bold;}
nav{width:100%; height: 50px;}
.acctSearchbar{ font-size:11px; height: 20px; border-radius: 15px; border: none; background-color: #f1f1f187; box-shadow: 1px 1px 1px 1px #bfbfbf6e; padding-left:8px;}
.acctSearchbar:focus{outline: none;}
.acctSearchBtn{ font-size: 11px; border: none; border-radius: 7px; height: 22px;  background-color: #f9dcdcd4;}
.acctSearchSet{ font-size:11px; height: 22px; border-radius: 5px; position: relative; top: 1px; left: 5px;}
...
<c:choose>
	...
	<c:otherwise>
	<div class="AcctMgtListOverlay">
	</div>
	<div class="AcctMgtListWrap">
	<script type="text/javascript">
...
//검색 기능
		...
		//검색 기능 실행 부분
		$('.acctSearchbar').keyup(function(event) {
			if(event.keyCode == 13){
				word = $('.acctSearchbar').val();
				option = $('.acctSearchSet option:selected').val();
...
//테이블 선택 구현 + 계좌 선택 구현
		...
		$(document).on('dblclick','.AcctMgtTable td', function() {
			code = select.children('input[name="acct_code"]').val();
			var name = select.parent().children('td:nth-child(3)').text();
...
//입력, 조회, 수정 팝업 닫기
		...
		<div class="wrap">
			...
			<nav>
				<div class="sort">
					<input class="acctSearchbar" type="text" name="acctSearchbar" value="" placeholder="Search">
					<input class="acctSearchBtn" type="button" name="acctSearchBtn" value="검색" onclick="">
					<select class="acctSearchSet">
